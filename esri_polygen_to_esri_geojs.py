...
def shpGeoJsonToJson(fields, path):
    ...
    for shp in arcpy.ListFiles("*.shp"):
        ...
        json_file = os.path.join(path, (os.path.splitext(shp)[0] + '_ESRI.js'))
        ...
        f_w.write('"geometryType": "esriGeometryPolygon",')
        ...
        with arcpy.da.SearchCursor(shape, fields) as cursor:
            for row in cursor:
                try:
                    ...
                    f_w.write('"geometry": {"rings":%s}' % geo_new)
                    if n != rowNum:
                        f_w.write('},')
                        n += 1
                    else:
